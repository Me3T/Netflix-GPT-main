...
const MovieInfo = ({ video, detail }) => {
  ...
  const time = `${Math.floor(detail?.runtime / 60)} h ${
    detail?.runtime % 60
  } min`;
  ...
  return !detail ? (
    ...
    <div className="">
      <iframe
        className="w-full md:h-[50%] aspect-video"
        src={
      ...
      <div className="flex justify-between">
        <div className="px-2 md:px-4 py-1 w-3/4 ">
          ...
          <p className="text-[9px] md:text-xs text-slate-400 pl-0.5 md:pl-2">
            {detail?.release_date.split("-").reverse().join("/")} &bull;&nbsp;
        ...
        <div className="px-2 md:px-4 py-1 md:py-2 w-1/8 md:w-1/4 flex items-start relative justify-between">
          {video && (
            <CopyToClipboard text={text} onCopy={handleCopy}>
              <button className="text-slate-400 text-[13px] md:text-xl px-2 cursor-pointer">
                <i class="fa-regular fa-share-from-square"></i>
              </button>
            </CopyToClipboard>
          )}
          {copy && (
            <div id="copied" className="absolute visible">
          ...
          <button
            ...
            className={
              !like
                ? "text-slate-400 text-[13px] md:text-xl px-2 cursor-pointer"
                : "text-red-600 text-xl px-2 cursor-pointer"
            }
          >
      ...
      <div className="">
        <div className="flex pl-4 py-0.5">
          ...
          <h4 className="text-xs md:text-sm text-slate-400 pl-12">
            <i>{detail?.tagline}</i>
          </h4>
        </div>
        <div className="pl-4 pr-2 pt-1 ">
          <h2 className="text-sm md:text-base font-semibold">Overview</h2>
          <p className="py-0.5 text-[10px] md:text-sm">
            {detail?.overview.split(" ").splice(0, 28).join(" ")} ...
          </p>
        </div>
      </div>
    </div>
